...
namespace graphics{
	...
	int draw_board_graphic(positioning::game_state state, vector<vector<CHAR_INFO>> & image_drawing){
	...
	int draw_board_labels(vector<vector<CHAR_INFO>> & image_drawing){
		for(int i = 0; i < 10; i++){ //Draw labels
			image_drawing[i*(kInSquareHeight + 1) + kInSquareHeight/2 + 1][kBoardDrawingWidth].Char.UnicodeChar = L"0123456789"[i];
	...
	int draw_board_turn_label(positioning::game_state state, vector<vector<CHAR_INFO>> & image_drawing){
	...
	int draw_eval_meter(positioning::game_state state, vector<vector<CHAR_INFO>> & image_drawing){
		float score = eval::eval_board(state);
		...
		for(int j = 0; j < kEvaluationMeterValueCharNum; j++){
			...
			else if(score == -std::numeric_limits<float>::max()){
				score_str = L"-inf";
			}
			else{
